	...
	def POST(self):	
		...
		if row_num >= 1000:
			...
			if verified.lower().strip() == 'yes':
				...
				cv.add_to_queue("Friend-Q", name, form.netid, num)				
				message = "SUCCESS!"
...
class EndPairStart(object):
	def GET(self):
		return render.end_pair_start(message = 1)

	def POST(self):
		form = web.input(password = 'password')
		if form.password == cv.secret([3,3,1]):
			raise web.seeother('98hwerfyvug97843q870fv870324fv')
		else:
			error = ["Incorrect password! Please try again."]
			return render.end_pair_start(message = error)
